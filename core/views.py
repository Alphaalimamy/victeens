...
from .models import ImpactStat, Testimonial, FAQ, Program,TeamMember, OrganizationProfile, CoreValue, Milestone
# from .forms import ContactForm
...
def about_view(request):
    """About page view"""
    # Get FAQ by category
    general_faqs = FAQ.objects.filter(category='general', is_active=True)
    donation_faqs = FAQ.objects.filter(category='donation', is_active=True)
    volunteer_faqs = FAQ.objects.filter(category='volunteer', is_active=True)
...
def programs_view(request):
    programs = Program.objects.filter(is_active=True).order_by('title')  # You can add more filters or ordering as needed
...
def volunteer_view(request):
    if request.method == 'POST':
        ...
        interests = request.POST.getlist('interests')  # Gets multiple checkbox values
        availability = request.POST.get('availability')
        message = request.POST.get('message')
        
        # Save to database (you'd need a Volunteer model)
        ...
        confirmation_subject = 'Thank You for Your Volunteer Application'
        confirmation_body = f"""
        Dear {first_name},
            ...
            try:
                send_mail(
                    ...
                    message=(
                        ...
                        f"Message:\n{contact.message}\n\n"
                        f"---\n"
                        f"This message was sent from the Victory Teens contact form.\n"
